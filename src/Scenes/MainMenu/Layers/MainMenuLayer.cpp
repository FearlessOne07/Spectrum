...
void MainMenuLayer::OnAttach()
{
  ...
  mainMenuPanel->onHide = [mainMenuPanel, this, layerFadeDuration]()
  {
    GetOwner()->Engine().Tweens->AddTween<float>(
        ...
        [mainMenuPanel](float alpha)
        { mainMenuPanel->GetRenderTransform().SetOpacity(alpha); },
        {
            .startValue = mainMenuPanel->GetRenderTransform().GetOpacity(),
            .endValue = 0,
            .duration = layerFadeDuration,
            .onTweenEnd = [=]()
            { mainMenuPanel->SetVisibilityOff(); },
        } //
    );
  };
  ...
  {
    GetOwner()->Engine().Tweens->AddTween<float>( //
        {container.get(), "alpha"}, [container](float alpha)
        { container->GetRenderTransform().SetOpacity(alpha); },
        {
  ...
  playButton->SetFontSize(55 * 10);
  playButton->SetPadding(10);
  playButton->onClick = [this]()
  { _mainMenu->Hide(); };
  // playButton->SetSprite(buttonSprite);
  playButton->SetTextColor(Base::Blue);
  playButton->SetBackgroundColor(Base::Green);

  playButton->onHover = {
      [=, this]() {                                   //
        GetOwner()->Engine().Tweens->AddTween<float>( //
            {playButton.get(), "font-size"}, [=](float size)
            { playButton->GetRenderTransform().SetFontScale(size); },
            {
  ...
  exitButton->SetFontSize(55 * 10);
  exitButton->SetHAlignment(Base::HAlign::Center);
  exitButton->SetVAlignment(Base::VAlign::Center);
  exitButton->SetPadding(10);
  exitButton->onClick = [this]()
  { GetOwner()->SetSceneTransition(Base::SceneRequest::Quit); };
  // exitButton->SetSprite(buttonSprite);
  exitButton->SetTextColor(Base::Blue);
  exitButton->SetBackgroundColor(Base::Green);
  exitButton->onHover = {
      [=, this]() {                                   //
        GetOwner()->Engine().Tweens->AddTween<float>( //
            {exitButton.get(), "font-size"}, [=](float size)
            { exitButton->GetRenderTransform().SetFontScale(size); },
            {
  ...
  auto bus = Base::SignalBus::GetInstance();
  bus->SubscribeSignal<ShipSelectionAbortedSignal>([this](std::shared_ptr<Base::Signal> sig)
                                                   { _mainMenu->Show(); });
}
